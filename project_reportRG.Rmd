...
The kernel density approach, used as an alternative to Gaussian log-likelihood ratios, was calculated in a similar manner in the sense that a separate univariate likelihood ratio was calculated for each column which remained in the initial LASSO which was used for dimension reduction. The main difference was that kernel density estimates, using the `density` function in `R` were used to estimate the density for each class rather than an assumed Gaussian density. While tests for normality bolstered support for assuming Gaussian distributions, the nature of the competition also led us to consider a data-based density estimate as an alternative.  

However, this also creates the possibility of overfitting the likelihood ratio estimates to the small sample size given in the training data. In fact, correlations between Gaussian log-likelihood ratios and kernel density log-likelihood ratios were relatively low; the two methods produced rather different likelihood ratio estimates. This is most likely due to the kernel density estimates being too sensitive to small "bumps" in the class-conditional densities, whereas the Gaussian log-likelihood ratios were more robust to noise present in the features.  
...
The nature of the data as seemingly independent random normal variables with means around 0 and standard deviations around 1 led us to also consider an unsupervised clustering approach on the likelihood ratio features to infer whether there were any discernable patterns within the data. Model-based clustering was performed on columns '33', '65', and the set of Gaussian log-likelihood ratios, removing the response from consideration. While some clusters appeared to identify patterns present in the data, when clusters were compared to the response classes, there were no clusters which were more highly associated with one class than the other. This lack of association led us to remove any clustering categorizations from consideration as a feature in our dataset.  
...
\kiegan{I will talk about random forest predictions here.}  
